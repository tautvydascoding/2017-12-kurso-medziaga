...
// ================== UPDATE doctor ===========================
// $nr - kuri gydytoja redaguojame
// $vardas - naujas vardas
// $pavarde - naujas vardas

function editDoctor($nr,$vardas,$pavarde){
         $vardas = mysqli_real_escape_string($vardas); // del saugumo
         $pavarde = mysqli_real_escape_string($pavarde); // del saugumo

    $sql_tekstas = "UPDATE doctors SET name='$vardas', lname='$pavarde' WHERE id=$nr; ";
// UPDATE doctors SET name='Mike', lname='Pukuotukas' WHERE id=8;
    mysqli_query( getprisijungimas(), $sql_tekstas);
        if (!$arpavyko) {
            echo "Erorr: redaguoti nepavyko nr.: $nr !!!! <br>".mysqli_error(getprisijungimas());
        }
}

// editDoctor(3, 'Onaite', 'Egliene');

// =========================KEICIAM TIK VARDA==============================
$x = getDoctor(3);
$x = mysqli_fetch_assoc($x);
editDoctor(3,'Marija', $x['lname'] );
